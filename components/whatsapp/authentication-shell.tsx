...
import { WhatsAppLogoAuth } from "../icons/whatsapp-logo-auth";
import { WhatsAppWordmark } from "../icons/whatsapp-wordmark";
import { LoginUpsellIllustration } from "../icons/login-upsell-illustration";
import { CheckSmall } from "../icons/check-small";
import { LockSmall } from "../icons/lock-small";
...
interface AuthenticationShellProps {
    mode?: "login" | "signup" | "forgot-password" | "update-password" | "signup-success";
}



export function AuthenticationShell({ mode = "login" }: AuthenticationShellProps) {
    return (
        <div className="min-h-[100svh] w-full bg-[#FCF5EB] text-[#3B4A54] font-sans selection:bg-[#25D366] selection:text-white pb-10">
            ...
            <div className="w-full max-w-[1000px] mx-auto px-4 lg:px-0 space-y-8">
                ...
                {/* Main Login Card */}
                <div className="bg-white border border-border rounded-[22px] p-6 lg:p-[52px]">
                    <div className="flex flex-col gap-12 lg:gap-16 md:flex-row">

                        {/* Left: Instructions */}
                        <div className="flex-1">
                            <h1 className="text-[30px] font-light text-[#1e2a32] mb-8">
                                {mode === 'login' ? 'Pasos para iniciar sesión' :
                                    mode === 'signup' ? 'Pasos para crear cuenta' :
                                        mode === 'forgot-password' ? 'Recupera tu cuenta' :
                                            mode === 'signup-success' ? 'Verifica tu cuenta' :
                                                'Actualiza tu contraseña'}
                            </h1>

                            <ol className="space-y-6">
                                {mode === 'forgot-password' || mode === 'update-password' || mode === 'signup-success' ? (
                                    <>
                                        <li className="flex gap-4 items-start">
                                            <div className="flex flex-col items-center">
                                                <span className="flex h-10 w-10 items-center justify-center rounded-full border border-[#c6d9dc] bg-white text-base font-semibold text-[#1e2a32] shadow-[0_0_0_1px_rgba(0,0,0,0.05)]">1</span>
                                        ...
                                        <li className="flex gap-4 items-start">
                                            <div className="flex flex-col items-center">
                                                <span className="flex h-10 w-10 items-center justify-center rounded-full border border-[#c6d9dc] bg-white text-base font-semibold text-[#1e2a32] shadow-[0_0_0_1px_rgba(0,0,0,0.05)]">2</span>
                                                <span className="mt-1 h-[calc(100%-1.5rem)] w-px bg-[#dfe7e8]" aria-hidden />
                                            </div>
                                            <span className="text-[17px] leading-6 text-[#3b4a54]">
                                                {mode === 'forgot-password'
                                        ...
                                        <li className="flex gap-4 items-start">
                                            <div className="flex flex-col items-center">
                                                <span className="flex h-10 w-10 items-center justify-center rounded-full border border-[#c6d9dc] bg-white text-base font-semibold text-[#1e2a32] shadow-[0_0_0_1px_rgba(0,0,0,0.05)]">3</span>
                                            </div>
                                            <span className="text-[17px] leading-6 text-[#3b4a54]">
                                                {mode === 'forgot-password'
                                    ...
                                    <>
                                        ...
                                        <li className="flex gap-4 items-start">
                                            <div className="flex flex-col items-center">
                                                <span className="flex h-10 w-10 items-center justify-center rounded-full border border-[#c6d9dc] bg-white text-base font-semibold text-[#1e2a32] shadow-[0_0_0_1px_rgba(0,0,0,0.05)]">4</span>
                                            </div>
                                            <span className="text-[17px] leading-6 text-[#3b4a54]">
                        ...
                        <div className="flex-shrink-0 md:w-[360px] lg:w-[400px]">
                            <div className="relative rounded-[28px] px-10 py-12 text-white border border-border">
                                {mode === "login" && <LoginForm />}
                                {mode === "signup" && <SignUpForm />}
                                {mode === "forgot-password" && <ForgotPasswordForm />}
                                {mode === "update-password" && <UpdatePasswordForm />}
                                {mode === "signup-success" && <SignUpSuccess />}
                            </div>

                            {mode === 'login' && (
                                <div className="mt-6 text-center text-sm text-[#54656f]">
                                    ¿Tienes problemas?{" "}
                                    <button className="text-[#008069] font-semibold hover:underline">
                                        Obtener ayuda
                                    </button>
                                </div>
                            )}
            ...
            <div className="w-full text-center mt-12 pb-8 px-4">
                <p className="text-[#3b4a54] text-[15px] mb-2 font-medium">¿No tienes una cuenta de WhatsApp? <Link href="/auth/sign-up" className="text-[#008069] hover:underline">Primeros pasos</Link></p>
